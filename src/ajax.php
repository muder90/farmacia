...
if (isset($_GET['q'])) {
    ...
    if ($insertar) {
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $stockActual = mysqli_fetch_assoc($stockActual);
			
			if (($stockActual['existencia']) >= ($cantidad))
			{
            $stockTotal = $stockActual['existencia'] - $cantidad;
...
if (isset($_POST['regDetalle'])) {
    ...
    if ($result > 0) {
		...
		$existencia = mysqli_fetch_assoc($query);
		if ($existencia['existencia'] >= $cant)
		{
		...
		$query = mysqli_query($conexion, "select codproducto, existencia from producto where codproducto = '".$id."' ");
		$existencia = mysqli_fetch_assoc($query);
		if ($existencia['existencia'] >= $cant)
		{	
